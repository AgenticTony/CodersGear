...
Retrieving Shop ID
List all the shops associated with your account by calling Shops API json [ { "id": 5432, "title": "My new store", "sales_channel": "My Sales Channel" }, { "id": 9876, "title": "My other new store", "sales_channel": "disconnected" } ]
...
View Response
Disconnect a shop
⚠ This endpoint requires a {shop_id} parameter. See Retrieving Shop ID for instructions on how to obtain your shop ID.
DELETE /v1/shops/{shop_id}/connection.json
Disconnect a shop
DELETE /v1/shops/{shop_id}/connection.json
View Response
...
⚠ All product endpoints require a {shop_id} parameter. See Retrieving Shop ID for instructions on how to obtain your shop ID.
On this page:
...
"brand_name": "Some Amazon approved brand name that has been set on the Printify store"
}
Etsy
"sales_channel_properties": {
"free_shipping": false,
...
"warehouse_id": "456",
"free_shipping": false,
"personalisation": "Personalisation instructions"
}
views
READ-ONLY
"views": [
...
⚠ All order endpoints require a {shop_id} parameter. See Retrieving Shop ID for instructions on how to obtain your shop ID.
⚠ On-the-fly product creation is a time-consuming operation. To ensure reliability, please create products in advance to avoid timeouts.
...
⚠ By default, new stores have automatic order approval enabled (24 hours). If auto-approval is active, orders will be sent to production automatically once imported/created, without calling the Send to production API endpoint.

If you prefer to control when orders are sent to production:
Check each store's Order approval settings and set them to Manual to avoid unintended production.
When ready, explicitly trigger production via the Send to production endpoint.
On this page:
...
The order contains items with shipping restricted line items
...
Unique identifier of the line item from the external system. Preserved during replacements and routing.
variant_id
REQUIRED
READ-ONLY
"variant_id": 17887
...
READ-ONLY
"external_id": "abc-1234"
The external identifier provided when submitting the order.
Metadata properties
order_type
READ-ONLY
"order_type": "external"
...
⚠ All webhook endpoints require a {shop_id} parameter. See Retrieving Shop ID for instructions on how to obtain your shop ID.
On this page:
...
REQUIRED
host=example.com
The expected host of the webhook’s URL. This parameter acts as a safeguard: the deletion will only succeed if the webhook’s URL host matches the value provided. This prevents accidental deletion of webhooks belonging to a different host.
Delete a webhook
